# gfind . -regex '.*_final_.*' -printf '%T@ %c %p\n' | sort -k 1n,1 -k 7 | cut -d' ' -f2- > 1.txt
# gsed 's/.*\.\/\(.*\)/\1/g' 1.txt > 2.txt
# gsed 's/[\^0-9]/~~__~~__/g' 2.txt > 3.txt
# gsed -i 's/.*~~__~~__.*//g' 3.txt > 4.txt
# gsed -i ':a;N;$!ba;s/\n\n//g' 4.txt > 5.txt
# gsed 's/.*\.\/\(.*\)/\1/g' allFilesWithTag.txt > numbersFromFileNames.txt
# gsed -i 's/.*\([0-9]\{5\}\).*/\1/g' numbersFromFileNames.txt
# gsed -i 's/[^0-9]//g' numbersFromFileNames.txt
# gsed -i 's/\(.*_abstraction\) \([0-9]\{5\}\) \(.*\)/\1_\2_\3/g' mv_commands.sh
...
gsed 's/.*\.\/\(.*\)/\1/g' allFilesWithTag.txt > filesWith~tag~AndNoNumber.txt
		# Would produce a list of file names without the paths, but it turns out I want the paths:
		# gsed 's/.*\/\(.*\)/\1/g' temp2.txt > filesWith~tag~AndNoNumber.txt
	# The following two lines are a nasty cluge around the fact that I haven't found a working gsed regex to delete all lines (from a text file) which include any number. TO DO: Fix this so it doesn't delete lines that include a number in a folder name (so, only in anything after the final forward slash / ).
gsed 's/[\^0-9]/~~__~~__/g' filesWith~tag~AndNoNumber.txt
gsed 's/.*~~__~~__.*//g' filesWith~tag~AndNoNumber.txt
...
# gsed 's/\(.*\/\)\(.*[0-9].*\)/\1~_-~-_~\2/g' filesWith~tag~AndNoNumber.txt