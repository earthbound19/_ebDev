# DESCRIPTION
# Sorts mediaWiki color tables by all possible HSL permutations without repetition (see NOTE comment below) of H(ue), S(aturation), and L(ightness), each permutation set being sorted by descending order, using Cygwin (~'nix) sed and sort.
# USAGE
...
# re http://stackoverflow.com/a/27783094/1397555
		...
		# <names.txt sed 's/\([0-9]\)$/\10\2/' | sort --field-separator='=' -n -k 2,2 forColorSortTest.txt
		# NERP THAT; TRY:
