; DESCRIPTION: produces many hue shift and (de)saturation variants, of any images in a directory passed to it (as a command-line parameter), using nconvert. This means that you may drag and drop a folder of images onto this script file to create hue and saturation shift variants for every image in that folder. It outputs the variants in subfolders named after the original, with the hs adjustment values noted in the file names.

; LICENSE: I release this script to the public domain. 05/27/2015 02:01:58 PM -RAH

; DEPENDENCIES: the Windows operating system, and the free (and open source) softwares AutoHotkey and nconvert. Both of those free programs must be installed and in your system %PATH%.

; USAGE: Drag-and-drop a directory onto this file (as mentioned), or run it from the command line with one parameter, which is a directory. A command-line example:
...
scanDir = %1%
		;MsgBox, scanDir value is %scanDir%
whetherHide = Hide
...
; if filenotexist . . .
;hueValuesArray := Object()
;hueValuesArray := ["-7","-13","-20","-34","-37","-41","-45","-51","-64","46","44","41","37","33","31","30","28","24","15","8"]
; else :
Loop, Read, %A_ScriptDir%\RGB_hueShiftValues.txt
{
	hueValuesArray.Insert(A_LoopReadLine)
}

Loop, Read, %A_ScriptDir%\RGB_saturationShiftValues.txt
{
...
variantOutputDir = %scanDir%\%InNameNoExt%_RGB_HSL_variants
