...
# - sed, svgo
# Prerequisites:
# - a run of svgo_optimize.sh (following the configuration instructions therein) against both color transplant recipient and donar svg files. svgo_optimize.sh is to get source and destination svgs into a format with all paths on one line and attributes sorted just so, so that the sed command in this script will even work on the files as expected. Specifically, all paths and fills must be attributes (not styles) in the svgs this operates on, with the path, and the id and fill attributes in this pattern: `<path id="path4524" fill="#eb6a06"` .. also, you must have and specify (by parameter, see USAGE) a folder which contains only the svg files you wish to transplant color from.
...
# sed has to be used instead of IFS / <<< thingy because the transplant source file name may not come with a pathname/file.svg structure (it may not be in a subfolder):
SVGcolorTransplantSRCfileNameNoPath=`echo $SVGcolorTransplantSRCfileName | sed 's/\(.*\/\)\(.*\)/\2/g'`
...
sed -n 's/.*<path id="\([^"]\{0,\}\)" fill="#\([a-fA-F0-9]\{6\}\)" .*/\1,\2/p' $SVGcolorTransplantSRCfileName > tmp_8cEX5EEK_transplantPathsAndFillsSRC.txt
...
do
	...
	sed -i "$sedCommand" $newSVGtransplantFileName
done
