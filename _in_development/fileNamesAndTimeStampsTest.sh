...
# gsed -i '/.*\/.*_final_.*/!d' filesWithTagAndNoNumber.txt
...
# gsed -i '/.*\/.*_[0-9]\{5\}_.*\|.*_[0-9]\{5\}\.[^0-9]\{1,4\}/!d' numbersFromFileNames.txt
	# Read those into an array:
	
# Golden ridiculous test gsed regex that got me on my way to solving a super hard problem; it isolates _final_ if it appears before a file name (in other words in the path) from _final_ if it appears after the path (in other words in a file name):
# gsed 's/\([^\/]*_final_[^\/]*\)\(\/[^\/]*\)/\1~\2/g' testSrc.txt
