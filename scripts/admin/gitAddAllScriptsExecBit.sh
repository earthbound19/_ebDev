# DESCRIPTION
# Searches for all executable type scripts in all subdirectories and runs a git command against each of them to `git add` them with the execute bit set. Solves a problem where files created and normally added to git on Windows don't have that, and some 'nixy envrionments properly refuse to execute them withuot that bit. (In fact, even if they are in your PATH, some or all 'nix environments behave as if they don't even exist if you try to locate them with `whereis scriptName.sh` or `whereis scriptName.sh` -- it doesn't even throw an error.)

# USAGE
# Hard-code the script types listing in the find command at the start of the script per your preferences, then invoke this script without any parameters:
