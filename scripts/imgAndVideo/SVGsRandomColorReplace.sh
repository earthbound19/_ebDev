# DESCRIPTION
# calls SVGrandomColorReplace.sh for every .svg file in the current directory, doing random fills from .hexplt file $1, replacing color $2 with random colors from $1. Uses a new svg file for $1 for that script, passing $1 from this as $2 and $2 from this as $3. Note that script is named SVGrandom~ where this script is SVGsRandom~, and this script repeatedly calls that one.
...
# An example that will randomly replace the color 919191 with a randomly chosen color from palette bg_rnd_fills.hexplt, for every .svg file in the current directory:
#    SVGsRandomColorReplace.sh bg_rnd_fills.hexplt 919191
