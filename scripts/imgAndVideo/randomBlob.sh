...
# USAGE
# randomblob.sh [options] [maskfile] outfile
...
#   -o osize      Output image size WxH or single value (default: 512)
#   -s shape      Inner region shape: square, disk (default: disk)
...
#   -S seed       Random seed (any positive integer, will be hashed if too large) (default: 1)
...
# Arguments:
#   maskfile      Optional mask image (white region on black background)
#   outfile       Output image file (PNG, GIF, etc.)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -*)
            ...
            ;;
        *)
            if [[ -z "$maskfile" && -f "$1" && "$outfile" == "" ]]; then
                maskfile="$1"
            elif [[ "$outfile" == "" ]]; then
                outfile="$1"
            else
...
if [[ "$debug" == "yes" ]]; then
    ...
    echo "  Output size: ${owidth}x${oheight}" >&2
    echo "  Shape: $shape" >&2
    echo "  Blur: $blur" >&2
...
if [[ "$save" == "yes" ]]; then
    cp "$tmp_canvas" "connected_${outfile}"
fi
