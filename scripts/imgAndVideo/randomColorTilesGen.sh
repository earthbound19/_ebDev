...
# $4 The multiplier scale you want to scale up the resulting image by (maintaining hard edges).
# $5 A file list of hex color values to randomly pick from (instead of doing "completely" pseudo-random colors).
# This script first creates a .ppm plain text format image file, then converts it to a .png file, maintaining hard edges and scaling up by the multiplier of the third parameter. Example command:
# thisScript.sh 20 10 15 80
...
# TO DO: convert from hex color lists thusly; operating on hex values after list import:
	...
	# -first checking for non-empty $5 and importing from list given by $5 thus:
	# if [ ! -z ${5+x} ]
...
do
	...
	# USUALLY UNUSED, on account of wanting to make mp4 animations where ffmpeg can upscale on the fly (and skip need of any upscaled .png image) :
