# DESCRIPTION
# Combines all input image files of type $1 (parameter 1 to script) into a layered output image file. Suggested options: ora, kra, or psd. ora (works with Krita and possibly other open raster-compatible programs) recommended and is default.
...
# - $1 The extension of the file type in your current directory to operate on, for example png, which would cause the script to combine all png images into the destination layered format, $2:
# - $2 OPTIONAL. The desired final layered file format, e.g. tif, kra or psd. If omitted, defaults to ora.
# Example run command:
...
# - It seems that if you have an instance of Krita already open, it may take longer or fail? In testing, closing Krita before running this script sped things up / unblocked.
...
if [ "$2" ]; then final_target=_all_"$1"_layered.$2; else final_target=_all_"$1"_layered.ora; fi
...
then
	echo "Compositing final image; depending, this may take a long time . . ."
