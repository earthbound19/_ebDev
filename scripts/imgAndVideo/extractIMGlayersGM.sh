# DESCRIPTION
# Extracts image layers from a Photoshop file to numbered, individual images.
# OR YOU MIGHT TRY: Photoshop -> File menu -> Export -> Layers to files (and the alpha may only look good if you export to png-24)! How long has that been there, and I bothered with coding this script?! Maybe it wasn't in the Photoshop version I had at the time? Also see NOTES. This script exports layers in an image file (e.g. psd or tif source file) to png images.
