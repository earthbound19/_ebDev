...
# support, and optional SVG output. Some switches renamed and made optional vs. original.
...
# USAGE
# python randomBlob.py [options]

# Basic image output with auto-generated filename
# python randomBlob.py -n 20 -l 15

# Basic image output with specified filename
# python randomBlob.py -n 20 -l 15 -o blob.png

# Specify dimensions (default 512)
# python randomBlob.py -n 20 -l 15 -d 1024

# With mask file
# python randomBlob.py -n 20 -l 15 -m mask.png

# SVG output with custom spline parameters
# python randomBlob.py -n 30 --drawtype spline -T 0.5 -C -0.2 -B 0.3 --typeoutput svg -o blob.svg

# Use 75% of available CPU cores
# python randomBlob.py -n 100 --cpu 75 -o complex_blob.png

# Save the cool scribble intermediate image
# python randomBlob.py -n 20 -l 15 --save-connected -o blob.png

# NOTES
# - This is a clean room reimplementation, dedicated to the Public Domain
# - All parameters match the original randomblob.sh switch names except where noted
...
# - Automatic filename generation when -o is omitted
# - Clean switch design: -d for dimensions, -o for output, -m for mask, --drawtype for connection method
...
def generate_auto_filename(args):
    """Generate an automatic filename based on parameters."""
    # Format: randomBlob_S<seed>_n<points>_p<pixel_increment>_l<linewidth>.<ext>
    base = f"randomBlob_py_S{args.S}_n{args.n}_p{args.p}_l{args.l}"
    
    # Determine extension based on output type
    if args.typeoutput == 'svg':
        ext = 'svg'
    elif args.typeoutput == 'both':
        ext = 'png'
    else:  # image
        ext = 'png'
...
def main():
    ...
    # Core Parameters (match original randomblob.sh interface where possible)
    ...
    parser.add_argument('-d', type=str, default='512', help='Output image dimensions (WxH or single value)')
    ...
    parser.add_argument('-m', type=str, help='Mask image file (white region on black background)')
    ...
    # Connection Method
    parser.add_argument('--drawtype', type=str, default='spline', choices=['line', 'spline'],
                       help='Connection method (default: spline)')
    ...
    parser.add_argument('-o', '--output', type=str, help='Output file path (auto-generated if omitted)')
    ...
    output_width, output_height = parse_size(args.d)
    if output_width <= 0 or output_height <= 0:
        raise ValueError("Output dimensions must be > 0")
    
    # Handle output file
    if args.output is None:
        # Need seed first for filename
        if args.S is None:
            args.S = random.randint(1, 2**32 - 1)
            print(f"Generated random seed: {args.S} (use -S {args.S} to reproduce this blob)", file=sys.stderr)
        args.output = generate_auto_filename(args)
        print(f"Auto-generating output filename: {args.output}", file=sys.stderr)
    ...
    debug_print(args.debug, f"Parameters: n={args.n}, l={args.l}, i={args.i}, d={output_width}x{output_height}")
    ...
    if args.m:
        try:
            mask_image = Image.open(args.m).convert('L')
    ...
    if args.typeoutput in ['svg', 'both']:
        ...
        if args.typeoutput == 'both':
            svg_file = os.path.splitext(args.output)[0] + '.svg'
        else:
    ...
    if args.typeoutput in ['image', 'both']:
        if args.typeoutput == 'both':
            image_file = os.path.splitext(args.output)[0] + '.png'
        else:
