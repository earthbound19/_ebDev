...
# DEPENDENCIES
# Requires a 'nix/Cygwin environment with nconvert (from xnconvert) installed and in the $PATH; likewise ffmpeg, also renumberFiles.sh from: https://github.com/earthbound19/_devtools

# USAGE
# Run this script. Wait (maybe a long time). The resultant video will be at:
# ./dist/[timestamp]__1MB_img_seq_representation.mp4
...
# if [ ! -d pbm ]; then mkdir pbm; fi

# mv *.pbm ./pbm/

# cd pbm

# renumberFiles.sh pbm

# TO DO: make this alter %0[n]d based on the number of .pbm image files it converts.
# timestamp=`date +"%Y_%m_%d__%H_%M_%S__%N"`
# ffmpeg -y -f image2 -i %03d.pbm -vf scale=1280:720:flags=neighbor -crf 6 -r 29.97 "$timestamp"__1MB_img_seq_representation.mp4

# cd ..

# if [ ! -d dist ]; then mkdir dist; fi
# mv pbm/"$timestamp"__1MB_img_seq_representation.mp4 ./dist
