...
# throw an error and exit if parameter $2 passed to script is greater than the number of available files (of extension $1) to copy:
if [ $sizeOfallFilesOfExtension -le $2 ]
then
	echo ERROR\: parameter \$2\, the requested number of files to copy of type $1 \(parameter \$1\) is greater than available number of such files\, $sizeOfallFilesOfExtension\ \(this script is not designed to avoid file name conflicts by e.g. duplicating the same source file to a new target name\)\.
	exit
fi
