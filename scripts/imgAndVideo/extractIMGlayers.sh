...
# NOTES: At this writing and perhaps every writing, this script may produce files with layer numbers not corresponding to layer numbers in the input file. Also it will crop layers with transparency and pixels not at all edges, and may offset (wrap) pixels :(
...
# perhaps kludgy, but all I can figure to do:
...
do
	gm convert $1[$i] "$imgFileNoExt"_layer"$i".png
done