...
# TO DO:
# - Fix bug: if -b is passed but not -c (and maybe in other circumstances), -b saves to preset with spaces between commas, which messes up loading the preset. Remove spaces from the string before saving.
# - Have reclaimOrphanedCoordinates do its work once when stopPaintingPercentAsDecimal is reached?
...
if args.reclaimOrphanedCoordinates:
	reclaimOrphanedCoordinates = args.stopPaintingPercentAsDecimal
else:
