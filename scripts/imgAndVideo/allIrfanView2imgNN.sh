...
# CODE
# Store a list of all files of type $1 in an array, first trimming off any leading ./ from the listed files via sed:
allParam1files=`find . -maxdepth 1 -iname "*.ppm" | sed 's/^\.\///g'`
