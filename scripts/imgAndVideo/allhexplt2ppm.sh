...
# - $3 OPTIONAL. number of tiles across of tiles-assembled image (columns).
...
do
	...
	echo ~~~~
	# Saves a needless run of the script this calls (dunno whether that's faster than just calling the script, which also checks for the target file and doesn't render if it exists) :
	if [ ! -f $targetFileName ]
	then
				echo Target file $targetFileName not found\; WILL RENDER.
				renderedCount=$(( $renderedCount + 1 ))
				echo Rendering ppm file \#$renderedCount of \#$filesToRender.
				echo Running hexplt2ppm.sh for $fileName.
