...
# .h264 and .avi files. Adapted from:
#    ffmpegAnim.sh.

# WARNING
# This script automatically overwrites _out.mp4 if it already exists.
...
# - $6 OPTIONAL. How many seconds to loop the last frame, to create a long still of the last frame appended to the end of the video. Creates the still loop as `_append.mp4`, then muxes `_out.mp4` and `_append.mp4` to a temp `mp4`, deletes both the originals and renames the temp to `_out.mp4`. KLUDGE: if you want to use this but not $5, pass the word NULL as $5.
# EXAMPLE
#    x264anim.sh 29.97 29.97 13 png
# NOTES
# - Search for the additionalParams options and uncomment or modify them (or don't) as you wish.
# - You can hack this script to produce an animated .gif image simply by changing the extension at the end of the applicable command line (third from last line of code?)
