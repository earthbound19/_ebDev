...
# USAGE
# First, run allPaletteCompareCIECAM02.sh as instructed in its comments. This is necessary to create the file paletteDifferenceRankings.txt, which this script relies on. Then, invoke with one parameter:
...
# Example invocation that will list every palette pair where the difference value is _below_ 0.065:
...
# count up by 0.1 increments from number until we reach 1, and with each increment, remove all lines from the list that start with that number:
		# first time I've done a while loop in bash; this continues so long as the expression contained in backticks ` evaluates to 1 (true):
