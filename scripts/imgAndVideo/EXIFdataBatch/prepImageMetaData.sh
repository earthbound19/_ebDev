...
captionSearchPath=`cygpath -u "D:\Alex\gibberish\IAE_generatedGibberish\pyMarkovGibbGen\edited"`
...
do
	...
	then
		...
		ls $captionSearchPath/*.txt > temp.txt
		...
		if [ -a ${gibberishTextFilenamesArray[0]} ]
			then
				...
				mv "${gibberishTextFilenamesArray[0]}" $captionSearchPath/used/$imageFileNameNoExt-caption.txt
				# metaDataAdditionsTextFile
# TO DO: prepend to the gibberish source text file a note about which file the EXIF data was inserted into.
			else
				echo no text file found in $captionSearchPath. Aborting script.
				exit
