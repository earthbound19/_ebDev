...
# - $3 OPTIONAL. New image X (pixels across) dimension or percent. Smaller strongly recommended. Aspect matching this many X pixels will be maintained. See details in img2img.sh. To not use this but use $4 (see next), pass the word NULL for $3.
# - $4 OPTIONAL. Anything, for example the word HOIHOI, which will cause the script to do conversion in all subfolders also.
