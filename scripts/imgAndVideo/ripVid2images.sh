# USAGE: $1 is the source file. $2 is a target dimension to resize to if any (BROKEN at this writing, do not use that parameter!) ; e.g. nnnnXnnnn or e.g. 320:-1 or -1:800; the latter two calculate and keep an aspect ratio automatically targeting a given X or Y pix dimension. e.g. 320:-1 will target 320x pixels and whatever corresponding Y pixels would keep the aspect, or -1:800 would target 800y pixels and whatever corresponding X pixels will keep the aspect.
...
# TO DO: add back correct (has it changed?) format for scale arg.: -vf scale=$2 
