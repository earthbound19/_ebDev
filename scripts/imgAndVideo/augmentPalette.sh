...
# FORMERLY: `get_color_gradient_OKLAB.js` (and nodeJS and the packages that script requires)
...
# A HACKABLE GLOBAL; notable alternative options are 'oklab' and 'oklch:
...
# if local environment variable fullPathToInterpolationScript is not set, set it. If it is set, assume it correct and don't alter it:
if [ ! "$fullPathToInterpolationScript" ]
then
		# DEPRECATED, because interpolateTwoSRGBColors_coloraide.py can do the same thing but with various colorspaces:
		# scriptName=get_color_gradient_OKLAB.js
	scriptName=interpolateTwoSRGBColors_coloraide.py
	fullPathToInterpolationScript=$(getFullPathToFile.sh $scriptName)
	# exit with error if that's empty:
	if [[ "$fullPathToInterpolationScript" == "" ]]; then echo "ERROR: could not find script $scriptName in \$PATH. Exit."; exit 3; fi
fi
# echo "Found $scriptName at $fullPathToInterpolationScript -- will use that."
...
do
	...
	augmentedPaletteSegment=(
		...
		-s $thisElement \
		-e $nextElement \
		-n $interpolationSteps $deduplicateAdjacentSamplesParameter \
		-l 1 \
