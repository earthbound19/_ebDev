# DESCRIPTION
# variant of RGBhexColorSortInCIECAM02.py which uses the color-science library and CAM16 color model for color sorting (supposedly superior to CIECAM02).
# In my tests, RGBhexColorSortInCIECAM02.py (which uses the colorspacious library) does better for hue matching independent of brightness; this (which uses colour-science) does better for matching first brightness/saturation, then hue. Both give good results. To my eye colorspacious is better overall (maybe I reckon hue before brightness/saturation).
...
#  Python RGBhexColorSortInCAM16-UCS.py PrismacolorMarkers.hexplt
# The result will be printed to stdout, so that you may for example redirect the output to a new .hexplt file, e.g.:
#  Python RGBhexColorSortInCAM16-UCS.py inputColors.hexplt > result.hexplt
...
#  Python RGBhexColorSortInCAM16-UCS.py inputColors.hexplt foo
...
# - Good comparisons with this script may be a matter of here figuring the best illuminant and transform matrix paramters, and hacking that. You can try different chromatic_adaptation_transform values from here:
...
# CODE
# See REFERENCE and dev notes section at end of script.
import sys
...
# may not be valid) ;
# Conditional print: only print this information if we're overwriting original palette file (because if we aren't ovewriting it, the user may be piping output to a new palette file that they wouldn't want cluttered with other information that breaks the file format:
