# DESCRIPTION
# Renames all files of a given extension (via parameter) in the path from which this script is called--renames them to zero-padded numbers matching the number of digit columns of the count of all said files. By default sorts files alphabetically before renumbering. Can be overridden to sort by oldest file first (-o) or newest file first (-n) before renumbering.

# WARNINGS
# - Use this only in directories where you actually want _all_ files of the given extension renamed by numbers.
# - POTENTIAL DATA LOSS IF MISUSED! If any of your file names are numeric-only (e.g. 005.png) *before* you run this script against them, files may disappear via overwrite, effectively erasing that file by replacing it with new content. For example, a file named 005.png may be overwritten when a file named someOtherFile.png is renamed to 005.png, overwriting the original file named 005.png -- and the data of the original 005.png would thereby be GONE FOREVER (effectively, permanently deleted). This script now includes collision detection to prevent such data loss. The script does checks to mitigate this, but beware regardless.
# - There are a variety of known potential issues at this writing; it has been tested and works for my purposes, but be aware of that.
...
# - -o --oldest-file-first OPTIONAL. Sort by oldest file first (ascending timestamp) before renumbering.
# - -n --newest-file-first OPTIONAL. Sort by newest file first (descending timestamp) before renumbering.
...
# Renumber all png format files in the current directory and all subdirectories, sorted by oldest file first:
#    renumberFiles.sh -e png -r -o
# Renumber all png format files in the current directory and all subdirectories, sorted by newest file first:
#    renumberFiles.sh -e png -r -n
...
function print_halp {
	echo "Usage: $0 -e EXTENSION [OPTIONS]"
	echo ""
	echo "Required:"
	echo "  -e, --extension EXT    File extension to operate on"
	echo ""
	echo "Options:"
	echo "  -r, --recurse          Process subdirectories recursively"
	echo "  -sNUM, --start-number=NUM  Start numbering at NUM (default: 0)"
	echo "  -o, --oldest-file-first   Sort by oldest file first (ascending timestamp)"
	echo "  -n, --newest-file-first   Sort by newest file first (descending timestamp)"
	echo "  -dNUM, --digits-to-pad-to=NUM  Pad numbers to NUM digits"
	echo "  -pSTRING, --prefix-string=STRING  String to insert before number"
	echo "  -xSTRING, --postfix-string=STRING  String to insert after number, before extension"
	echo "  -h, --help             Show this help"
	echo ""
	echo "Note: -o and -n are mutually exclusive. If neither is specified, files are"
	echo "      sorted alphabetically by filename."
	echo ""
	echo "IMPORTANT: For options that require a value (-s, -d, -p, -x), the value must be"
	echo "attached directly to the option with no space (e.g., -s42, not -s 42)."
	echo "See comments at top of script for detailed documentation."
}

# print help and exit if no parameters passed:
if [ ${#@} == 0 ]; then print_halp; exit 0; fi

PROGNAME=$(basename $0)

# Pre-scan validation for options that require values
# This catches cases where user does: -x value (space) instead of -xvalue
requires_arg_options=("-x" "-p" "-d" "-s")
i=0
while [ $i -lt $# ]; do
    arg="${!i}"
    for opt in "${requires_arg_options[@]}"; do
        if [ "$arg" = "$opt" ]; then
            # Check if next argument exists and isn't another option
            next_idx=$((i + 1))
            if [ $next_idx -ge $# ]; then
                echo "ERROR: Option $opt requires a value but no value provided." >&2
                echo "       Use ${opt}value or ${opt}'value' (no space between option and value)." >&2
                exit 1
            fi
            next_arg="${!next_idx}"
            if [[ "$next_arg" =~ ^- ]]; then
                echo "ERROR: Option $opt requires a value but next argument '$next_arg' appears to be another option." >&2
                echo "       Use ${opt}value or ${opt}'value' (no space between option and value)." >&2
                exit 1
            fi
        fi
...
OPTS=`getopt -o he:rs:ond:p:x: --long help,extension:,recurse,start-number:,oldest-file-first,newest-file-first,digits-to-pad-to:,prefix-string:,postfix-string: -n $PROGNAME -- "$@"`
...
while true; do
  case "$1" in
    ...
    -n | --newest-file-first ) sort_by_newest_first=true; shift ;;
...
if [ ! "$?" == "0" ]
then
	echo "-s --start-number parameter ($startCountingFrom) not an integer. Exit."; exit 4
fi
...
then
	...
	if [ ! "$?" == "0" ]
	then
		echo "-d --digits-to-pad-to parameter ($digitsToPadTo) not an integer. Exit."; exit 5
	fi
fi

# Check mutual exclusivity of -o and -n
if [ "$sort_by_oldest_first" = true ] && [ "$sort_by_newest_first" = true ]; then
    echo "ERROR: Options -o/--oldest-file-first and -n/--newest-file-first are mutually exclusive." >&2
    echo "Please choose one or the other." >&2
    exit 6
fi
...
# echo sort_by_newest_first is $sort_by_newest_first
...
do
	...
	temp_counter=$startCountingFrom
	...
	for filename in "${filesArray[@]}"; do
		...
		rename_map["$filename"]="$renameTarget"
		
		# Check for collision if target exists and is different from source
		if [ -e "$renameTarget" ] && [ "$filename" != "$renameTarget" ]; then
			collisions+=("File '$filename' would overwrite existing file '$renameTarget'")
		fi
	done
	
	# If collisions found, report and exit without making changes
	if [ ${#collisions[@]} -gt 0 ]; then
		echo "ERROR: The following collisions would occur and cause data loss:" >&2
		printf '  - %s\n' "${collisions[@]}" >&2
		echo "No files were renamed in directory '$directory'." >&2
		echo "Please resolve these conflicts and try again." >&2
		popd &>/dev/null
		exit 2
	fi
	
	# SECOND PASS: No collisions found, proceed with actual renaming
	fileRenumberingCounter=$startCountingFrom
	for filename in "${filesArray[@]}"; do
...
# DEVELOPMENT HISTORY
# 2026-03-27 Added -n/--newest-file-first option with mutual exclusivity with -o. Added pre-scan validation for options requiring values (space detection). Added collision detection to prevent data loss via overwrites
