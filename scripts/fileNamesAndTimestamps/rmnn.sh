...
# - $3 OPTIONAL. End count of files to delete. If omitted, all files of type $1 after count $2 are deleted (and so is file at count $2).
# EXAMPLES
...
if [ ! "$3" ]; then endCount=$array_length; else endCount=$3; fi
...
do
	...
	echo "DELETING file idx $seq, file name $fileNameToDelete . . ."
	rm $fileNameToDelete
done
