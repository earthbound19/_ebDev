...
gsed '/.*_[0-9]\{5\}_.*\|.*_[0-9]\{5\}\.[^0-9]\{1,4\}/!d' ./_batchNumbering/PartB_originalFiles.txt > ./_batchNumbering/numbersFromFileNames.txt
	# Reduce those results to numbers only (no text):
		# USED THE FOLLOWING; two different examples that will be put together into one hecka ugly | compount regex:
			# gsed -i 's/.*_\([0-9]\{5\}\)_.*/\1/g' ./_batchNumbering/numbersFromFileNames.txt
			# The dev iteration that got me there for everything after the | in the next active line of code:
			# gsed 's/.*_\([0-9]\{5\}\)\(.*\.[^\.]\{1,6\}\)/\1__\2~~/g' ./_batchNumbering/numbersFromFileNames.txt
...
gsed -i 's/.*_\([0-9]\{5\}\)\(.*\.[^\.]\{1,6\}\)/\1/g' ./_batchNumbering/numbersFromFileNames.txt
		...
		# NERP:
		# gsed -i 's/.*_\([0-9]\{5\}\)\(.*\.[^\.]\{1,6\}\)|.*_\([0-9]\{5\}\)\(.*\.[^\.]\{1,6\}\)/\1/g' ./_batchNumbering/numbersFromFileNames.txt
