...
# USAGE
# Copy a list of exclusion words to `excludeThese.txt`. Copy the words you want edited (to exclude all words from `excludeThese.txt`) to `fromThese.txt`. Then run this script:
#    excludeWords.sh
# The results will appear in fromThese_parsed.txt.
