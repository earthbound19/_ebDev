...
// - If controlling booleans are set true, click/drag also causes PNG/SVG image save (only saves first clicked frame and last frame of variant.)
// - See other global variables as documented below for other functionality.
// NOTES
// - If saveAllFrames is true, SVG animation frames are saved to _anims/ subfolder, grouping by number for this run of the program, variation, an rnd string, and SVG files named by frame. The SVGs may be copied out of these subfolders into one folder, and they will be incrementally numbered IF saveAllFrames was true for the entire program run. if that boolean was toggled off and on during run, the SVG file names won't be strictly contiguous, but will still be ordered by higher number last.
// - You can dynamically load external palette files and use them for random selection of background, light shape fill, dark shape fill, and stroke (outline) color. These palettes have the .hexplt extension, and are in the /data subfolder. The file format is a list of RGB color codes (in hexadecimal format), one color code per line preceded by #. Whether it loads from these palette files is controlled by the global boolean variable loadColorsFromHexplts (hard-coded default set to = true). Set loadColorsFromHexplts to false and the program will only use the internal hard-coded color palettes, which are more varied. The external palettes are hard-coded to more pastel colors, sorted into background, light, dark, and stroke colors. You may alter the color lists in those external files to your liking to more quickly try different colors (more quickly than finding the hard-coded color lists in the source code and hacking them).
// - You can either use only the hard-coded palettes, or load from the external palettes and use those, or randomly alternate between both. To use the hardcoded, set loadColorsFromHexplts to false. To use the external, set loadColorsFromHexplts to true and rndSwapPalettes (you'll find that varaible nearby) to false. To randomly alternate between both, set loadColorsFromHexplts to true and set rndSwapPalettes to true (which is the default hard-coded).
// - The colorMode global variable controls color modes, and works this way: if hard-coded to 1, the program will render everything in full color. If set to 2, everything will be rendered grayscale. If set to 3, everything will be rendered in the three override background, fill and stroke colors set in altBackgroundColor, altFillColor, and altStrokeColor. However, by default, it reverts to color mode when every time the program creates and renders a new variation (which you can manually do by pressing the forward (right) arrow key on the keyboard. You can also go back one variation with the back (left) arrow key. To have it keep the same color mode through every variation, set keepColorModeOnVariantChange to true.
// - You can double-click (or for a kiosk or tablet, double-tap on the screen) to cycle color modes in this order: color -> grayscale -> color override -> back to color.
...
// CODE
// by Richard Alexander Hall 2019 all rights reserved.

// v2.3.14 work log:
// Comment fixup for collating documentation.
String versionString = "v2.3.14";

// if the following boolean is set to true, then:
// do simple file names/dropbox folder save + tweet; ALSO, the program will override tryToTweet to true; AND fullscreen will override to true.
// if false, complex file names and local save:
boolean museumMode = false;

// TO DO; * = doing:
// * ATTEMPT MADE; may need to work off new fake size/dist values in object: (concentricity control and) rnd higher/lower concentricity range.
...
// TO ENABLE IMAGE TWEETS, uncomment the last section of code with a varaible named simpletweet, uncomment the desired code, then work backward to hunt for the related code earleir that defines simpletweet until you get back to the define which follows these comments, then run this script and see if it doesn't complain of undefined things.
// I don't want to require that library to be installed just to run this script, so that's how I'm managing disabling / enabling tweet dependency.
// NOTE that this requires the presence of a text file in the same folder, named twitterAPIauth.txt, containing OAauth keys that will work with twitter's API. Without those the tweet will fail and the console and _tweetErrorLog.txt will note the failure.
...
// TO DO: remove && saveSVGs == true as part of condition? (saveAllAnimationFrames overrides?) :
  ...
  if (currentTimeMilliseconds >= runSetupAtMilliseconds) {
    // IF WE DON'T DO THE FOLLOWING, this block here is run immediately in the
