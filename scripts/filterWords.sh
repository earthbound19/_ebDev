...
# CODE
if [ ! "$1" ]; then printf "\nNo parameter \$1 (file name of text file with filter words) passed to script. Exit."; exit 1; else filterWordsFileName=$1; fi
...
awk '{if (f==1) { r[$0] } else if (! ($0 in r)) { print $0 } } ' f=1 $filterWordsFileName f=2 $fileNameToFilterWordsFrom