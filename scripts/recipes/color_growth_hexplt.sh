# DESCRIPTION
# repeatedly invokes color_growth.py, with a new --RANDOM_SEED and one custom --CUSTOM_COORDS_AND_COLORS coordinate/color each run; the colors being from a .hexplt file passed to the script; the other thing per preferences customizable via parameters. Also, it creates stub .rendering files when it starts a render, to enable concurrent runs (of more than one instance of this script running at the same time, but without duplicating work). See USAGE.

# USAGE
# Invoke with at least three required parameters, and optional additional parameters:
...
# - $4 OPTIONAL. Either A) the keyword RND_BASIC_POS or B) a pair of x,y values (a component of that expected by color_growth.py's --CUSTOM_COORDS_AND_COLORS), surrounded by single quote marks, for example '1799,1799'. (Note that while the --CUSTOM_COORDS_AND_COLORS switch expects parenthesis around those values (among other things), it may break it to do that here. Don't use parenthesis here). If not provided, the x,y values are highly randomized (read on). If you need to use $5 but don't want to "use" $4 (leave it at default), pass the keyword NULL as $4.
# - $5 OPTIONAL. Any additional arguments that can be accepted by color_growth.py, surrounded by single (or double?) quote marks, e.g. '--WIDTH 3600 --HEIGHT 3600 --SAVE_EVERY_N 0'. If not provided no additional parameters are used (the variable that controls them is not even set; it is effectively empty as far as the script is concerend). If provided, you may be able to use --WIDTH and --HEIGHT parameters in it if they wouldn't cause errors vs. $2 and $3. (I suppose only if in $5 they are larger than $2 and $3?) If you want to use $6 but not $5, pass the keyword NULL as $5.
...
# Example invocation with only a palette, and width 1920 x 1080:
#  color_growth_hexplt.sh RAHfavoriteColorsHex.hexplt 1920 1080
# See comments in hexplt2rgbplt for expected PATH for any .hexplt file ($1).
...
# Example invocation with the same paramaters but also using the RND_BASIC_POS keyword for $4:
#  color_growth_hexplt.sh collectedColors1.hexplt 1920 1080 RND_BASIC_POS
# Example invocation that would cause the script to use a specific origin coordinate for every render (not RND_BASIC_POS):
#  color_growth_hexplt.sh collectedColors1.hexplt 1920 1080 '(2027,400)'
# Example invocation that would cause the script to choose a random coordinate from the whole canvas range each render:
#  color_growth_hexplt.sh collectedColors1.hexplt 1920 1080 NULL
# Example invocation that passes custom parameters (with different values than previous examples for the other parameters):
...
if [ "$4" ] && [ "$4" == "RND_BASIC_POS" ]; then coordinateSuperParameter=$4; printf "\nParameter 2 passed to the script as \'$coordinateSuperParameter\'; using that."; fi
if [ "$4" ] && [ "$4" != "RND_BASIC_POS" ] && [ "$4" != "NULL" ]; then coordinateSuperParameter=$4; printf "\nParameter 2 passed to the script as \'$coordinateSuperParameter\'; using that."; fi

if [ "$5" ] && [ "$5" != "NULL" ]; then extraArgs=$5; printf "\n\nParameter \$5 passed to script as \'$extraArgs\'. Using that."; fi
