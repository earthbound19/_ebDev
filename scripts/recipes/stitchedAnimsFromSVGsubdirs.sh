# DESCRIPTION
# Makes a stitched anim from output subdirectories of SVG animation
# frames, from small_and_simple_things.pde (Processing) output with e.g.
# the following global variables set to these values:
...
# CODE
array=(`gfind . -maxdepth 1 -type d -printf '%f\n' | sort -n`)
